...
export default class OfficePreviewPlugin extends Plugin {
  ...
  private addStyles() {
    ...
    style.textContent = `
      .op-pptx-preview { height: 100%; display: flex; flex-direction: column; }
      .op-pptx-header { padding: 16px 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; font-weight: 600; display: flex; justify-content: space-between; align-items: center; }
      ...
      .op-pptx-toc { width: 220px; background: #f8f8f8; border-right: 1px solid #e0e0e0; overflow-y: auto; padding: 12px 0; }
      ...
      .op-pptx-nav { padding: 14px 20px; background: white; border-top: 1px solid #e0e0e0; display: flex; justify-content: center; align-items: center; gap: 20px; }
  ...
  async openPptxPreview(file: TFile) {
    // Create a new leaf (split view or tab)
    ...
    try {
      ...
      const tocItems = slides.map((s, i) => 
        `<div class="op-pptx-toc-item ${i === 0 ? 'active' : ''}" data-index="${i}">${s.title || '第 ' + (i+1) + ' 页'}</div>`
      ).join('');
      ...
      leaf.view.containerEl.innerHTML = `
        <div class="op-pptx-preview">
          ...
          <div class="op-pptx-body">
            <div class="op-pptx-toc">
              <div class="op-pptx-toc-title">📑 幻灯片</div>
              ${tocItems}
            </div>
            <div class="op-pptx-content">
              ${slideContents}
            </div>
          </div>
        ...
        <script>
        window.opPptxData = { slides: ${slides.length} };
        function opPptxNav(dir) {
          var preview = document.querySelector('.op-pptx-preview');
          var cur = 0;
          preview.querySelectorAll('.op-pptx-slide').forEach(function(s, i) { 
            if (s.classList.contains('active')) cur = i; 
          });
      ...
      new Notice(`已打开: ${file.name}`);
    } catch (e: any) {
      leaf.view.containerEl.innerHTML = `<div class="op-pptx-error">❌ ${e.message}</div>`;
    }
  }
  ...
  private inflate(data: Uint8Array): Uint8Array {
    if (typeof window !== 'undefined' && (window as any).pako) return (window as any).pako.inflate(data);
    const result: number[] = [];
    let i = 0;
    while (i < data.length) {
      const b = data[i];
  ...
  private extractContent(xml: string): string[] {
    const items: string[] = [];
